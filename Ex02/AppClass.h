...
class Application
{
	vector3 m_v3Pumpkin; //position of the creeper
	Model* m_pPumpkin = nullptr; //model to display
	Model* m_pEnderman = nullptr; //model to display
	MyRigidBody* m_pPumpkinRB = nullptr; //Rigid Body of the model

	std::vector<Model*> m_modelList; //Models to display
	std::vector<MyRigidBody*> m_RigidBodyList; //Models to display
	std::vector<matrix4> m_modelMatrixList; //Matrices to display objects

	bool m_bCameraSwitch = false;
		
	String m_sProgrammer = "Nik Barbero - nfb1799@rit.edu"; //programmer
...
public:
	void RenderFromInsidePumpkin(matrix4 m4PumpkinToWorld);
